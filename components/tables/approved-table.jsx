...
import { MoreHorizontal, ArrowUpDown, CheckCircle, XCircle, Eye, EyeOff } from "lucide-react"
import { useEffect, useState } from "react"
import api from "@/lib/axios"
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "@/components/ui/dropdown-menu"
import { formatDummyValue } from "@/lib/dummy-data"

export default function ApprovedTable({fetchInvoices, onRefresh}) {
  ...
  const handleRejectInvoice = async (invoiceId) => {
    try {
      await api.put(`/invoices/${invoiceId}/reject`)
  ...
  if (loading) {
    return (
      <div className="p-8 text-center">
        ...
        <p className="mt-2 text-gray-600">Loading approved invoices...</p>
      </div>
    )
  }
          ...
          <div className="min-w-max">
            <Table className="w-full">
              ...
              <TableBody>
                {invoices?.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={showAllColumns ? 13 : 7} className="text-center py-8 text-gray-500">
                  ...
                  invoices?.map((row, index) => (
                    <TableRow key={index} className="border-b border-[#e4e4e7] hover:bg-[#f7f7f7]">
                      <TableCell className="text-[#49454f] p-6 min-w-[140px]">
                      ...
                      <TableCell className="p-4 w-12">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => handleRejectInvoice(row.id)}>
              ...
              <div key={index} className="bg-white border border-[#e4e4e7] rounded-lg p-4 space-y-3">
                ...
                <div className="flex justify-end pt-2">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => handleRejectInvoice(invoice.id)}>
                        <XCircle className="w-4 h-4 mr-2" />
                        Reject
                      </DropdownMenuItem>
                    </DropdownMenuContent>
